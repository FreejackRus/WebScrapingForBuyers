...
- компактный topbar (вордмарк ≤118px, бейдж, регион-chip с ellipsis, аватар 44px;
  без wrap/logout/коннекторов в шапке; 52px высота).
- мобильные настройки: короткие вкладки, одна колонка полей, выход в savebar,
  не в шапке.
- вкладка AI Копилот скрывает поиск и таблицу, оставляя композер;
- поиск: поле + «Найти» в один ряд без карточки-обёртки, 16px input, tap ≥44px;
- копайлот на Радаре компактный; вкладка — на весь экран, без длинного lead;
- карточки офферов: источник → цена → товар → условия → «К офферу»;
