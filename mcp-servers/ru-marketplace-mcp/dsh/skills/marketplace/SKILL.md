...
`lamoda_*`, `dns_*`, `citilink_*`, `aliexpress_*`, `cian_*` plus
...
wires a single `marketplace` entry instead of thirteen. The server exposes 38
tools: 37 mounted plus `marketplace_sources`.
