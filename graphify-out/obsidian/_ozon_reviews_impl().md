---
source_file: "mcp-servers/ru-marketplace-mcp/packages/ozon-connector/src/ozon_connector/server.py"
type: "code"
community: "Community 152"
location: "L874"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_152
---

# _ozon_reviews_impl()

## Connections
- [[BadRequestError]] - `uses` [INFERRED]
- [[Context_1]] - `references` [EXTRACTED]
- [[OzonReviewsResponse]] - `calls` [EXTRACTED]
- [[ParserDriftError]] - `uses` [INFERRED]
- [[TransportDownError]] - `uses` [INFERRED]
- [[_canonical_product_path_from_input()]] - `calls` [EXTRACTED]
- [[_fetch_composer()]] - `calls` [EXTRACTED]
- [[_ozon_blocked_error()]] - `calls` [EXTRACTED]
- [[_parse_review_item()]] - `calls` [EXTRACTED]
- [[_safe_review_page_path()]] - `calls` [EXTRACTED]
- [[_widget()]] - `contains` [EXTRACTED]
- [[cdp_setup_hint()]] - `calls` [EXTRACTED]
- [[ozon_connectorserver.py]] - `contains` [EXTRACTED]
- [[ozon_reviews implementation — kept separate so the public tool can wrap it in a…]] - `rationale_for` [EXTRACTED]
- [[ozon_reviews()]] - `calls` [EXTRACTED]
- [[raise_tool_error()]] - `calls` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_152