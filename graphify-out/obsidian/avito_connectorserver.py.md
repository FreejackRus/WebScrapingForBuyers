---
source_file: "mcp-servers/ru-marketplace-mcp/packages/avito-connector/src/avito_connector/server.py"
type: "code"
community: "json"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/json
---

# avito_connector/server.py

## Connections
- [[Avito MCP connector. Avito sits behind an IP-reputation firewall a datacenter…]] - `rationale_for` [EXTRACTED]
- [[AvitoCardResponse]] - `imports` [EXTRACTED]
- [[AvitoSearchItemOut]] - `imports` [EXTRACTED]
- [[AvitoSearchResponse]] - `imports` [EXTRACTED]
- [[AvitoSelfcheckResponse]] - `imports` [EXTRACTED]
- [[AvitoSellerOut]] - `imports` [EXTRACTED]
- [[AvitoSellerResponse]] - `imports` [EXTRACTED]
- [[MetaOut]] - `imports` [EXTRACTED]
- [[_avito_selfcheck_impl()]] - `contains` [EXTRACTED]
- [[_blocked_error()]] - `contains` [EXTRACTED]
- [[_build_search_url()]] - `contains` [EXTRACTED]
- [[_cdp_fetch()]] - `contains` [EXTRACTED]
- [[_extract_item_id()]] - `contains` [EXTRACTED]
- [[_fetch()]] - `contains` [EXTRACTED]
- [[_looks_like_json()]] - `contains` [EXTRACTED]
- [[_parse_search_items()]] - `contains` [EXTRACTED]
- [[_polite_wait()]] - `contains` [EXTRACTED]
- [[_posted_at()]] - `contains` [EXTRACTED]
- [[_proxy()]] - `contains` [EXTRACTED]
- [[_raise_for_fetch_failure()]] - `contains` [EXTRACTED]
- [[_sync_curl_get()]] - `contains` [EXTRACTED]
- [[_valid_location_id()]] - `contains` [EXTRACTED]
- [[asyncio_2]] - `imports` [EXTRACTED]
- [[avito_card()]] - `contains` [EXTRACTED]
- [[avito_connectormodels_output.py]] - `imports_from` [EXTRACTED]
- [[avito_connectorsettings.py]] - `imports_from` [EXTRACTED]
- [[avito_connectorshape_reference.py]] - `imports_from` [EXTRACTED]
- [[avito_search()]] - `contains` [EXTRACTED]
- [[avito_search.py]] - `imports_from` [EXTRACTED]
- [[avito_selfcheck()]] - `contains` [EXTRACTED]
- [[avito_seller()]] - `contains` [EXTRACTED]
- [[cache.py]] - `imports_from` [EXTRACTED]
- [[chrome_cdp.py]] - `imports_from` [EXTRACTED]
- [[curl_cffi]] - `imports_from` [EXTRACTED]
- [[datetime]] - `imports` [EXTRACTED]
- [[errors.py]] - `imports_from` [EXTRACTED]
- [[fastmcp_3]] - `imports_from` [EXTRACTED]
- [[fastmcp_server_middleware_error_handling]] - `imports_from` [EXTRACTED]
- [[get_settings()_3]] - `imports` [EXTRACTED]
- [[json]] - `imports` [EXTRACTED]
- [[logging.py]] - `imports_from` [EXTRACTED]
- [[mcp_core__init__.py]] - `imports_from` [EXTRACTED]
- [[mcp_types]] - `imports_from` [EXTRACTED]
- [[missing_required_families()]] - `imports` [EXTRACTED]
- [[output_schema.py]] - `imports_from` [EXTRACTED]
- [[pacing.py]] - `imports_from` [EXTRACTED]
- [[pydantic]] - `imports_from` [EXTRACTED]
- [[re]] - `imports` [EXTRACTED]
- [[redact.py]] - `imports_from` [EXTRACTED]
- [[test_live_payload_contract.py]] - `imports_from` [EXTRACTED]
- [[typing]] - `imports_from` [EXTRACTED]
- [[urllib_parse]] - `imports` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/json