---
source_file: "mcp-servers/ru-marketplace-mcp/packages/compare-connector/src/compare_connector/identity.py"
type: "code"
community: "Community 30"
location: "L61"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_30
---

# normalize_gtin()

## Connections
- [[Any_14]] - `references` [EXTRACTED]
- [[Return a valid GTIN-8121314, or empty for missinginvalid input.]] - `rationale_for` [EXTRACTED]
- [[_text()_1]] - `calls` [EXTRACTED]
- [[identity.py]] - `contains` [EXTRACTED]
- [[identity_from_mapping()]] - `calls` [EXTRACTED]
- [[match_product_identity()]] - `calls` [EXTRACTED]
- [[test_gtin_is_normalized_and_check_digit_validated()]] - `calls` [EXTRACTED]
- [[test_gtin_rejects_malformed_values()]] - `calls` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_30