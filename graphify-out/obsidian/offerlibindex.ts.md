---
source_file: "apps/web/src/entities/offer/lib/index.ts"
type: "code"
community: "offers/ui/index.tsx"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/offers/ui/indextsx
---

# offer/lib/index.ts

## Connections
- [[OfferSort]] - `contains` [EXTRACTED]
- [[OfferSortColumn]] - `contains` [EXTRACTED]
- [[OfferSortDirection]] - `contains` [EXTRACTED]
- [[applyTableFilter()]] - `contains` [EXTRACTED]
- [[compareText()]] - `contains` [EXTRACTED]
- [[conditionLabels]] - `contains` [EXTRACTED]
- [[filterOffers()]] - `contains` [EXTRACTED]
- [[matchLabels]] - `contains` [EXTRACTED]
- [[matchRank]] - `contains` [EXTRACTED]
- [[nextOfferSort()]] - `contains` [EXTRACTED]
- [[offerindex.ts]] - `re_exports` [EXTRACTED]
- [[offerSortValue()]] - `contains` [EXTRACTED]
- [[packages_contracts_dist_index]] - `imports_from` [EXTRACTED]
- [[packages_contracts_dist_index_offer]] - `imports` [EXTRACTED]
- [[packages_contracts_dist_index_offertablefilter]] - `imports` [EXTRACTED]
- [[sortOffers()]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/offers/ui/indextsx