---
source_file: "apps/search/src/infrastructure/sources/demo-source-adapter.ts"
type: "code"
community: "Community 9"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_9
---

# demo-source-adapter.ts

## Connections
- [[DemoSourceAdapter]] - `contains` [EXTRACTED]
- [[DemoSourceOptions]] - `contains` [EXTRACTED]
- [[SourceAdapter]] - `imports` [EXTRACTED]
- [[createDemoSources()]] - `contains` [EXTRACTED]
- [[packages_contracts_dist_index]] - `imports_from` [EXTRACTED]
- [[packages_contracts_dist_index_matchkind]] - `imports` [EXTRACTED]
- [[packages_contracts_dist_index_offer]] - `imports` [EXTRACTED]
- [[packages_contracts_dist_index_product]] - `imports` [EXTRACTED]
- [[packages_contracts_dist_index_productcondition]] - `imports` [EXTRACTED]
- [[ref_node_crypto]] - `imports_from` [EXTRACTED]
- [[ref_node_timers]] - `imports_from` [EXTRACTED]
- [[searchsrcapp.ts]] - `imports_from` [EXTRACTED]
- [[source-adapter.ts]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_9