---
source_file: "apps/web/package.json"
type: "concept"
community: "App.tsx"
location: "L16"
tags:
  - graphify/concept
  - graphify/EXTRACTED
  - community/Apptsx
---

# react

## Connections
- [[App.tsx]] - `imports_from` [EXTRACTED]
- [[AppShell.tsx]] - `imports_from` [EXTRACTED]
- [[LoginShell.tsx]] - `imports_from` [EXTRACTED]
- [[WorkspaceShell.tsx]] - `imports_from` [EXTRACTED]
- [[chatuiindex.tsx]] - `imports_from` [EXTRACTED]
- [[commanduiindex.tsx]] - `imports_from` [EXTRACTED]
- [[copilotuiindex.tsx]] - `imports_from` [EXTRACTED]
- [[login-carduiindex.tsx]] - `imports_from` [EXTRACTED]
- [[main.tsx]] - `imports_from` [EXTRACTED]
- [[offer-tableindex.ts]] - `imports_from` [EXTRACTED]
- [[useBootSession.ts]] - `imports_from` [EXTRACTED]
- [[useSearchHotkey.ts]] - `imports_from` [EXTRACTED]
- [[webpackage.json]] - `imports` [EXTRACTED]
- [[widgetsusersettingsuiindex.tsx]] - `imports_from` [EXTRACTED]

#graphify/concept #graphify/EXTRACTED #community/Apptsx