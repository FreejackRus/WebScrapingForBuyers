---
source_file: "mcp-servers/ru-marketplace-mcp/packages/ozon-connector/tests/test_server.py"
type: "code"
community: "ozon-connector/tests/test_server.py"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/ozon-connector/tests/test_serverpy
---

# ozon-connector/tests/test_server.py

## Connections
- [[_patch_tier1()]] - `contains` [EXTRACTED]
- [[_reviews_body()]] - `contains` [EXTRACTED]
- [[_run()_1]] - `contains` [EXTRACTED]
- [[asyncio_2]] - `imports` [EXTRACTED]
- [[clear_cache()]] - `contains` [EXTRACTED]
- [[datetime]] - `imports_from` [EXTRACTED]
- [[fastmcp_exceptions]] - `imports_from` [EXTRACTED]
- [[json]] - `imports` [EXTRACTED]
- [[os]] - `imports` [EXTRACTED]
- [[ozon_connector__init__.py]] - `imports_from` [EXTRACTED]
- [[pathlib]] - `imports_from` [EXTRACTED]
- [[pydantic]] - `imports_from` [EXTRACTED]
- [[pytest]] - `imports` [EXTRACTED]
- [[test_cache_is_keyed_by_canonical_path_not_raw_input()]] - `contains` [EXTRACTED]
- [[test_canonical_composer_path_rejects_unsafe_search_query_keys()]] - `contains` [EXTRACTED]
- [[test_canonical_product_path_ignores_relative_query_digits()]] - `contains` [EXTRACTED]
- [[test_card_and_reviews_accept_search_slug_product_url()]] - `contains` [EXTRACTED]
- [[test_cdp_fetch_json_times_out_open_page_and_releases_lock()]] - `contains` [EXTRACTED]
- [[test_fetch_composer_caches_a_successful_cdp_body()]] - `contains` [EXTRACTED]
- [[test_fetch_composer_does_not_cache_a_block()]] - `contains` [EXTRACTED]
- [[test_fetch_composer_reports_cdp_navigation_block_as_blocked()]] - `contains` [EXTRACTED]
- [[test_fetch_composer_serves_a_repeat_read_from_cache()]] - `contains` [EXTRACTED]
- [[test_fetch_composer_times_out_blocking_curl_and_falls_back_to_cdp()]] - `contains` [EXTRACTED]
- [[test_ozon_proxy_falls_back_to_standard_variables()]] - `contains` [EXTRACTED]
- [[test_ozon_search_reports_actionable_cdp_block()]] - `contains` [EXTRACTED]
- [[test_ozon_search_skips_unhashable_sku_values()]] - `contains` [EXTRACTED]
- [[test_ozon_selfcheck_includes_runtime_identity()]] - `contains` [EXTRACTED]
- [[test_ozon_tools_reject_non_object_payloads()]] - `contains` [EXTRACTED]
- [[test_parse_search_tile_canonicalizes_absolute_ozon_product_link()]] - `contains` [EXTRACTED]
- [[test_parse_search_tile_exposes_canonical_card_input_for_slug_link()]] - `contains` [EXTRACTED]
- [[test_parse_search_tile_ignores_hostile_nested_shapes()]] - `contains` [EXTRACTED]
- [[test_parse_search_tile_non_dict_returns_empty()]] - `contains` [EXTRACTED]
- [[test_parse_search_tile_non_list_main_state_no_crash()]] - `contains` [EXTRACTED]
- [[test_parse_search_tile_omits_unsafe_or_non_product_links()]] - `contains` [EXTRACTED]
- [[test_parse_search_tile_reads_textds_tile_name()]] - `contains` [EXTRACTED]
- [[test_reviews_marks_partial_when_later_page_fails()]] - `contains` [EXTRACTED]
- [[test_reviews_report_zero_own_reviews_when_the_pool_is_all_neighbours()]] - `contains` [EXTRACTED]
- [[test_reviews_tag_each_item_with_the_variant_it_describes()]] - `contains` [EXTRACTED]
- [[test_reviews_tolerate_drifted_item_id_and_products_shapes()]] - `contains` [EXTRACTED]
- [[test_reviews_tolerates_malformed_uuid_next_button_and_score()]] - `contains` [EXTRACTED]
- [[test_run_sync_bounded_rejects_local_callables()]] - `contains` [EXTRACTED]
- [[test_safe_review_page_path_rejects_non_string_next_button()]] - `contains` [EXTRACTED]
- [[test_server_version_matches_pyproject()]] - `contains` [EXTRACTED]
- [[test_sync_call_in_process_redacts_url_userinfo_from_child_errors()]] - `contains` [EXTRACTED]
- [[test_sync_call_in_process_scrubs_child_env()]] - `contains` [EXTRACTED]
- [[test_sync_call_in_process_times_out_and_next_call_still_works()]] - `contains` [EXTRACTED]
- [[test_sync_curl_get_closes_non_context_manager_response()]] - `contains` [EXTRACTED]
- [[test_tier1_proxy_is_passed_as_an_argument_not_an_env_var()]] - `contains` [EXTRACTED]
- [[time]] - `imports` [EXTRACTED]
- [[tomllib]] - `imports` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/ozon-connector/tests/test_serverpy