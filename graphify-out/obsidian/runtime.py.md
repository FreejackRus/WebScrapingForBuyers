---
source_file: "mcp-servers/ru-marketplace-mcp/packages/mcp-core/src/mcp_core/runtime.py"
type: "code"
community: "Community 67"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_67
---

# runtime.py

## Connections
- [[BearerAuthMiddleware]] - `contains` [EXTRACTED]
- [[FastMCP_2]] - `imports_from` [EXTRACTED]
- [[Transport selection shared by all connector entry points. Every connector is…]] - `rationale_for` [EXTRACTED]
- [[TransportConfig]] - `contains` [EXTRACTED]
- [[_parse_host()]] - `contains` [EXTRACTED]
- [[_parse_path()]] - `contains` [EXTRACTED]
- [[_parse_port()]] - `contains` [EXTRACTED]
- [[_parse_transport()]] - `contains` [EXTRACTED]
- [[_warn_if_exposed()]] - `contains` [EXTRACTED]
- [[browser_handoff_lifespan()]] - `contains` [EXTRACTED]
- [[collections_abc]] - `imports_from` [EXTRACTED]
- [[compare_connectorserver.py]] - `imports_from` [EXTRACTED]
- [[contextlib]] - `imports_from` [EXTRACTED]
- [[current_mcp_session_id()]] - `contains` [EXTRACTED]
- [[dataclasses]] - `imports_from` [EXTRACTED]
- [[fastmcp_server_dependencies]] - `imports_from` [EXTRACTED]
- [[fastmcp_server_middleware]] - `imports_from` [EXTRACTED]
- [[lamoda_connectorserver.py]] - `imports_from` [EXTRACTED]
- [[log_event()]] - `imports` [EXTRACTED]
- [[logging.py]] - `imports_from` [EXTRACTED]
- [[os]] - `imports` [EXTRACTED]
- [[resolve_transport()]] - `contains` [EXTRACTED]
- [[run_server()]] - `contains` [EXTRACTED]
- [[secrets]] - `imports` [EXTRACTED]
- [[sys]] - `imports` [EXTRACTED]
- [[taobao_connectorserver.py]] - `imports_from` [EXTRACTED]
- [[typing]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_67