---
source_file: "mcp-servers/ru-marketplace-mcp/packages/mcp-core/src/mcp_core/domtest.py"
type: "code"
community: "run_extractor"
location: "L125"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/run_extractor
---

# run_extractor()

## Connections
- [[8б. Ревизия собственной работы]] - `references` [INFERRED]
- [[Any_25]] - `references` [EXTRACTED]
- [[Execute ``js_source`` against ``html_path`` in jsdom and return its output.…]] - `rationale_for` [EXTRACTED]
- [[JsdomUnavailable]] - `calls` [EXTRACTED]
- [[Path_10]] - `references` [EXTRACTED]
- [[_extract()_11]] - `calls` [EXTRACTED]
- [[_extract()_3]] - `calls` [EXTRACTED]
- [[_extract()_12]] - `calls` [EXTRACTED]
- [[_extract()_14]] - `calls` [EXTRACTED]
- [[_extract()_13]] - `calls` [EXTRACTED]
- [[_extract()_1]] - `calls` [EXTRACTED]
- [[_extract()_10]] - `calls` [EXTRACTED]
- [[_extract()_2]] - `calls` [EXTRACTED]
- [[_extract()_15]] - `calls` [EXTRACTED]
- [[_extract()_6]] - `calls` [EXTRACTED]
- [[_extract()_9]] - `calls` [EXTRACTED]
- [[_extract()_4]] - `calls` [EXTRACTED]
- [[_extract()_7]] - `calls` [EXTRACTED]
- [[_extract()_8]] - `calls` [EXTRACTED]
- [[_extract()_5]] - `calls` [EXTRACTED]
- [[_extract()]] - `calls` [EXTRACTED]
- [[_items()]] - `calls` [EXTRACTED]
- [[_items()_3]] - `calls` [EXTRACTED]
- [[_node()]] - `calls` [EXTRACTED]
- [[_resolve_jsdom()]] - `calls` [EXTRACTED]
- [[_run()_2]] - `calls` [EXTRACTED]
- [[_wall_payload()]] - `calls` [EXTRACTED]
- [[domtest.py]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/run_extractor