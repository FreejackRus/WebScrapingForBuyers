---
source_file: "mcp-servers/ru-marketplace-mcp/packages/mcp-core/src/mcp_core/transport/browser_handoff.py"
type: "code"
community: "json"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/json
---

# browser_handoff.py

## Connections
- [[Bounded, process-local ownership of browser challenge tabs. No browser storage…]] - `rationale_for` [EXTRACTED]
- [[HandoffBusyError]] - `contains` [EXTRACTED]
- [[NotFoundError]] - `imports` [EXTRACTED]
- [[PageLike]] - `imports` [EXTRACTED]
- [[TransportDownError]] - `imports` [EXTRACTED]
- [[UpstreamTimeoutError]] - `imports` [EXTRACTED]
- [[_Lease]] - `contains` [EXTRACTED]
- [[_Request]] - `contains` [EXTRACTED]
- [[_duration_s()]] - `contains` [EXTRACTED]
- [[_env_seconds()]] - `contains` [EXTRACTED]
- [[_expired()]] - `contains` [EXTRACTED]
- [[_idle_s()]] - `contains` [EXTRACTED]
- [[_key()]] - `contains` [EXTRACTED]
- [[_max_leases()]] - `contains` [EXTRACTED]
- [[_payload_digest()]] - `contains` [EXTRACTED]
- [[_resume_note()]] - `contains` [EXTRACTED]
- [[_resume_summary()]] - `contains` [EXTRACTED]
- [[_run()]] - `contains` [EXTRACTED]
- [[_stop()]] - `contains` [EXTRACTED]
- [[asyncio_2]] - `imports` [EXTRACTED]
- [[chrome_cdp.py]] - `imports_from` [EXTRACTED]
- [[close_handoffs()]] - `contains` [EXTRACTED]
- [[collections_abc]] - `imports_from` [EXTRACTED]
- [[dataclasses]] - `imports_from` [EXTRACTED]
- [[datetime]] - `imports_from` [EXTRACTED]
- [[errors.py]] - `imports_from` [EXTRACTED]
- [[get_handoff_id()]] - `contains` [EXTRACTED]
- [[handoff_diagnostics()]] - `contains` [EXTRACTED]
- [[has_pending_handoff()]] - `contains` [EXTRACTED]
- [[hashlib]] - `imports` [EXTRACTED]
- [[json]] - `imports` [EXTRACTED]
- [[lamoda_connectorserver.py]] - `imports_from` [EXTRACTED]
- [[math]] - `imports` [EXTRACTED]
- [[os]] - `imports` [EXTRACTED]
- [[read_with_handoff()]] - `contains` [EXTRACTED]
- [[secrets]] - `imports` [EXTRACTED]
- [[snapshot_handoff()]] - `contains` [EXTRACTED]
- [[taobao_connectorserver.py]] - `imports_from` [EXTRACTED]
- [[time]] - `imports` [EXTRACTED]
- [[transport__init__.py]] - `imports_from` [EXTRACTED]
- [[typing]] - `imports_from` [EXTRACTED]
- [[urllib_parse]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/json