---
source_file: "mcp-servers/ru-marketplace-mcp/packages/marketplace-connector/tests/test_dependency_parity.py"
type: "code"
community: "test_dependency_parity.py"
location: "L57"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/test_dependency_paritypy
---

# _declared_dependencies()

## Connections
- [[Bare distribution names from project.dependencies, specifiers stripped.]] - `rationale_for` [EXTRACTED]
- [[_pyproject()]] - `calls` [EXTRACTED]
- [[test_dependency_parity.py]] - `contains` [EXTRACTED]
- [[test_every_connector_dependency_is_mounted_or_a_known_profile()]] - `calls` [EXTRACTED]
- [[test_every_mounted_source_is_a_declared_dependency()]] - `calls` [EXTRACTED]
- [[test_the_mounts_table_was_read()]] - `calls` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/test_dependency_paritypy