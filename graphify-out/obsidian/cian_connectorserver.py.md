---
source_file: "mcp-servers/ru-marketplace-mcp/packages/cian-connector/src/cian_connector/server.py"
type: "code"
community: "cian_connector/server.py"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/cian_connector/serverpy
---

# cian_connector/server.py

## Connections
- [[Cian MCP connector real-estate search and offer cards. Cian (cian.ru) is a…]] - `rationale_for` [EXTRACTED]
- [[CianAgentOut]] - `imports` [EXTRACTED]
- [[CianCardResponse]] - `imports` [EXTRACTED]
- [[CianMetroOut]] - `imports` [EXTRACTED]
- [[CianPriceChangeOut]] - `imports` [EXTRACTED]
- [[CianSearchItemOut]] - `imports` [EXTRACTED]
- [[CianSearchResponse]] - `imports` [EXTRACTED]
- [[CianSelfcheckResponse]] - `imports` [EXTRACTED]
- [[MetaOut_3]] - `imports` [EXTRACTED]
- [[_address()]] - `contains` [EXTRACTED]
- [[_agent()]] - `contains` [EXTRACTED]
- [[_as_float()]] - `contains` [EXTRACTED]
- [[_b()]] - `contains` [EXTRACTED]
- [[_blocked_error()_2]] - `contains` [EXTRACTED]
- [[_build_json_query()]] - `contains` [EXTRACTED]
- [[_cdp_page_config()]] - `contains` [EXTRACTED]
- [[_cdp_post_json()_1]] - `contains` [EXTRACTED]
- [[_cian_selfcheck_impl()]] - `contains` [EXTRACTED]
- [[_clean_url()]] - `contains` [EXTRACTED]
- [[_compose_title()]] - `contains` [EXTRACTED]
- [[_d()]] - `contains` [EXTRACTED]
- [[_extract_offer_id()]] - `contains` [EXTRACTED]
- [[_fetch_card()]] - `contains` [EXTRACTED]
- [[_fetch_search()]] - `contains` [EXTRACTED]
- [[_looks_like_json()_1]] - `contains` [EXTRACTED]
- [[_metro_rows()]] - `contains` [EXTRACTED]
- [[_offer_row()]] - `contains` [EXTRACTED]
- [[_parse_card()]] - `contains` [EXTRACTED]
- [[_parse_offers()]] - `contains` [EXTRACTED]
- [[_polite_wait()_5]] - `contains` [EXTRACTED]
- [[_price_history()]] - `contains` [EXTRACTED]
- [[_price_of()]] - `contains` [EXTRACTED]
- [[_price_unit()]] - `contains` [EXTRACTED]
- [[_raise_for_fetch_failure()_1]] - `contains` [EXTRACTED]
- [[_range()]] - `contains` [EXTRACTED]
- [[_rooms()]] - `contains` [EXTRACTED]
- [[_s()]] - `contains` [EXTRACTED]
- [[_validate_region()]] - `contains` [EXTRACTED]
- [[_views()]] - `contains` [EXTRACTED]
- [[asyncio_2]] - `imports` [EXTRACTED]
- [[cache.py]] - `imports_from` [EXTRACTED]
- [[chrome_cdp.py]] - `imports_from` [EXTRACTED]
- [[cian_card()]] - `contains` [EXTRACTED]
- [[cian_connectormodels_output.py]] - `imports_from` [EXTRACTED]
- [[cian_connectorsettings.py]] - `imports_from` [EXTRACTED]
- [[cian_search()]] - `contains` [EXTRACTED]
- [[cian_selfcheck()]] - `contains` [EXTRACTED]
- [[datetime]] - `imports` [EXTRACTED]
- [[errors.py]] - `imports_from` [EXTRACTED]
- [[fastmcp_3]] - `imports_from` [EXTRACTED]
- [[fastmcp_exceptions]] - `imports_from` [EXTRACTED]
- [[fastmcp_server_middleware_error_handling]] - `imports_from` [EXTRACTED]
- [[get_settings()]] - `imports` [EXTRACTED]
- [[json]] - `imports` [EXTRACTED]
- [[logging.py]] - `imports_from` [EXTRACTED]
- [[mcp_core__init__.py]] - `imports_from` [EXTRACTED]
- [[mcp_types]] - `imports_from` [EXTRACTED]
- [[output_schema.py]] - `imports_from` [EXTRACTED]
- [[pacing.py]] - `imports_from` [EXTRACTED]
- [[pydantic]] - `imports_from` [EXTRACTED]
- [[re]] - `imports` [EXTRACTED]
- [[redact.py]] - `imports_from` [EXTRACTED]
- [[typing]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/cian_connector/serverpy