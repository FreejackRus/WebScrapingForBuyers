---
source_file: "mcp-servers/ru-marketplace-mcp/packages/mcp-core/src/mcp_core/transport/chrome_cdp.py"
type: "code"
community: "chrome_cdp.py"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/chrome_cdppy
---

# chrome_cdp.py

## Connections
- [[Authenticated transport drive the operator's own Chrome over CDP. Several…]] - `rationale_for` [EXTRACTED]
- [[NavBlocked]] - `contains` [EXTRACTED]
- [[NavigationPolicyError]] - `contains` [EXTRACTED]
- [[PageLike]] - `contains` [EXTRACTED]
- [[_CdpConnectTimeout]] - `contains` [EXTRACTED]
- [[_RawCdpPage]] - `contains` [EXTRACTED]
- [[_WsLike]] - `contains` [EXTRACTED]
- [[_browser_ws_url()]] - `contains` [EXTRACTED]
- [[_cdp_port_open()]] - `contains` [EXTRACTED]
- [[_check_final_host()]] - `contains` [EXTRACTED]
- [[_chrome_candidates()]] - `contains` [EXTRACTED]
- [[_default_profile_dir()]] - `contains` [EXTRACTED]
- [[_ensure_cdp_running()]] - `contains` [EXTRACTED]
- [[_evaluate_expression()]] - `contains` [EXTRACTED]
- [[_find_chrome()]] - `contains` [EXTRACTED]
- [[_handoff_jpeg()]] - `contains` [EXTRACTED]
- [[_hide_chrome_windows()]] - `contains` [EXTRACTED]
- [[_host_from_env()]] - `contains` [EXTRACTED]
- [[_new_tab()]] - `contains` [EXTRACTED]
- [[_playwright_page()]] - `contains` [EXTRACTED]
- [[_port_from_env()]] - `contains` [EXTRACTED]
- [[_raw_cdp_page()]] - `contains` [EXTRACTED]
- [[_raw_page_count()]] - `contains` [EXTRACTED]
- [[_running_as_root()]] - `contains` [EXTRACTED]
- [[_scraping_profile_pids()]] - `contains` [EXTRACTED]
- [[_start_chrome_with_cdp()]] - `contains` [EXTRACTED]
- [[aliexpress_connectorserver.py]] - `imports_from` [EXTRACTED]
- [[asyncio_2]] - `imports` [EXTRACTED]
- [[avito_connectorserver.py]] - `imports_from` [EXTRACTED]
- [[base64]] - `imports` [EXTRACTED]
- [[binascii]] - `imports` [EXTRACTED]
- [[browser_handoff.py]] - `imports_from` [EXTRACTED]
- [[capture_owned_viewport()]] - `contains` [EXTRACTED]
- [[cdp_budget.py]] - `imports_from` [EXTRACTED]
- [[cdp_setup_hint()]] - `contains` [EXTRACTED]
- [[cian_connectorserver.py]] - `imports_from` [EXTRACTED]
- [[citilink_connectorserver.py]] - `imports_from` [EXTRACTED]
- [[collections_abc]] - `imports_from` [EXTRACTED]
- [[contextlib]] - `imports_from` [EXTRACTED]
- [[current_page_url()]] - `contains` [EXTRACTED]
- [[diagnose_drift.py]] - `imports_from` [EXTRACTED]
- [[dns_connectorserver.py]] - `imports_from` [EXTRACTED]
- [[get_browser()]] - `contains` [EXTRACTED]
- [[get_context()]] - `contains` [EXTRACTED]
- [[json]] - `imports` [EXTRACTED]
- [[lamoda_connectorserver.py]] - `imports_from` [EXTRACTED]
- [[math]] - `imports` [EXTRACTED]
- [[megamarket_connectorserver.py]] - `imports_from` [EXTRACTED]
- [[navigation_budget()]] - `imports` [EXTRACTED]
- [[open_page()]] - `contains` [EXTRACTED]
- [[os]] - `imports` [EXTRACTED]
- [[ozon_connectorserver.py]] - `imports_from` [EXTRACTED]
- [[pathlib]] - `imports_from` [EXTRACTED]
- [[playwright_async_api]] - `imports_from` [EXTRACTED]
- [[probe_session()]] - `contains` [EXTRACTED]
- [[re]] - `imports` [EXTRACTED]
- [[reveal_owned_page()]] - `contains` [EXTRACTED]
- [[shutil]] - `imports` [EXTRACTED]
- [[socket]] - `imports` [EXTRACTED]
- [[subprocess]] - `imports` [EXTRACTED]
- [[sys]] - `imports` [EXTRACTED]
- [[taobao_connectorserver.py]] - `imports_from` [EXTRACTED]
- [[time]] - `imports` [EXTRACTED]
- [[typing]] - `imports_from` [EXTRACTED]
- [[urllib_parse]] - `imports_from` [EXTRACTED]
- [[wb_connectorserver.py]] - `imports_from` [EXTRACTED]
- [[websockets]] - `imports` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/chrome_cdppy