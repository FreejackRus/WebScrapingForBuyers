---
source_file: "mcp-servers/ru-marketplace-mcp/packages/taobao-connector/tests/test_card_extractor_live_dom.py"
type: "code"
community: "test_card_extractor_live_dom.py"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/test_card_extractor_live_dompy
---

# test_card_extractor_live_dom.py

## Connections
- [[The Taobao card extractor against a LIVE captured item page.…]] - `rationale_for` [EXTRACTED]
- [[_extract()_8]] - `contains` [EXTRACTED]
- [[dom.py]] - `imports_from` [EXTRACTED]
- [[domtest.py]] - `imports_from` [EXTRACTED]
- [[pathlib]] - `imports_from` [EXTRACTED]
- [[pytest]] - `imports` [EXTRACTED]
- [[taobao_connector__init__.py]] - `imports_from` [EXTRACTED]
- [[test_live_description_images_are_counted()]] - `contains` [EXTRACTED]
- [[test_live_price_is_assembled_from_the_split_spans()]] - `contains` [EXTRACTED]
- [[test_live_shop_and_sales_are_the_short_nodes_not_body_blobs()]] - `contains` [EXTRACTED]
- [[test_live_title_is_the_product_name_not_a_widget_placeholder()]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/test_card_extractor_live_dompy