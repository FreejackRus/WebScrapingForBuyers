---
source_file: "mcp-servers/ru-marketplace-mcp/packages/megamarket-connector/tests/test_server.py"
type: "rationale"
community: "Community 42"
location: "L271"
tags:
  - graphify/rationale
  - graphify/EXTRACTED
  - community/Community_42
---

# Empty on a canary is a session problem, and must not read as drift. Reporting…

## Connections
- [[test_selfcheck_calls_an_empty_canary_not_authenticated()]] - `rationale_for` [EXTRACTED]

#graphify/rationale #graphify/EXTRACTED #community/Community_42