---
source_file: "apps/web/src/app/App.tsx"
type: "code"
community: "Community 64"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_64
---

# App.tsx

## Connections
- [[App()]] - `contains` [EXTRACTED]
- [[AppShell()]] - `imports` [EXTRACTED]
- [[AppView]] - `imports` [EXTRACTED]
- [[LoginPage()]] - `imports` [EXTRACTED]
- [[LoginShell()]] - `imports` [EXTRACTED]
- [[Monitor()]] - `imports` [EXTRACTED]
- [[Settings()]] - `imports` [EXTRACTED]
- [[apps_web_src_pages_search_index_searchmonitorpage]] - `imports` [EXTRACTED]
- [[apps_web_src_pages_user_index_settingspage]] - `imports` [EXTRACTED]
- [[compositionsettingsindex.ts]] - `imports_from` [EXTRACTED]
- [[entitiesuserindex.ts]] - `imports_from` [EXTRACTED]
- [[hooksindex.ts]] - `imports_from` [EXTRACTED]
- [[layerindex.ts]] - `imports_from` [EXTRACTED]
- [[main.tsx]] - `imports_from` [EXTRACTED]
- [[pagessearchindex.ts]] - `imports_from` [EXTRACTED]
- [[pagesuserindex.ts]] - `imports_from` [EXTRACTED]
- [[react_1]] - `imports_from` [EXTRACTED]
- [[useBootSession()]] - `imports` [EXTRACTED]
- [[useSearchHotkey()]] - `imports` [EXTRACTED]
- [[useUserStore]] - `imports` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_64