---
source_file: "mcp-servers/ru-marketplace-mcp/scripts/check_no_print.py"
type: "code"
community: "Community 125"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_125
---

# check_no_print.py

## Connections
- [[Fail if a connector server writes to stdout. An MCP stdio server owns stdout…]] - `rationale_for` [EXTRACTED]
- [[StdoutWriteVisitor]] - `contains` [EXTRACTED]
- [[_is_sys()]] - `contains` [EXTRACTED]
- [[_is_sys_stderr()]] - `contains` [EXTRACTED]
- [[_is_sys_stdout()]] - `contains` [EXTRACTED]
- [[ast]] - `imports` [EXTRACTED]
- [[check_file()]] - `contains` [EXTRACTED]
- [[collect_default_paths()]] - `contains` [EXTRACTED]
- [[main()_1]] - `contains` [EXTRACTED]
- [[pathlib]] - `imports_from` [EXTRACTED]
- [[sys]] - `imports` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_125