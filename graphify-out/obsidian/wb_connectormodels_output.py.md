---
source_file: "mcp-servers/ru-marketplace-mcp/packages/wb-connector/src/wb_connector/models_output.py"
type: "code"
community: "wb_connector/server.py"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/wb_connector/serverpy
---

# wb_connector/models_output.py

## Connections
- [[MetaOut_5]] - `contains` [EXTRACTED]
- [[Pydantic output models for the WB connector (Stage 2). Every tool returns a…]] - `rationale_for` [EXTRACTED]
- [[WbCardItem]] - `contains` [EXTRACTED]
- [[WbCardResponse]] - `contains` [EXTRACTED]
- [[WbCategoriesResponse]] - `contains` [EXTRACTED]
- [[WbCategoryNode]] - `contains` [EXTRACTED]
- [[WbCategoryProductsResponse]] - `contains` [EXTRACTED]
- [[WbNoResultsResponse]] - `contains` [EXTRACTED]
- [[WbQuestionItem]] - `contains` [EXTRACTED]
- [[WbQuestionsResponse]] - `contains` [EXTRACTED]
- [[WbReviewItem]] - `contains` [EXTRACTED]
- [[WbReviewsResponse]] - `contains` [EXTRACTED]
- [[WbRootInfoResponse]] - `contains` [EXTRACTED]
- [[WbSearchResponse]] - `contains` [EXTRACTED]
- [[WbSelfCheckEntry]] - `contains` [EXTRACTED]
- [[WbSelfCheckResponse]] - `contains` [EXTRACTED]
- [[WbSellerResponse]] - `contains` [EXTRACTED]
- [[models.py]] - `imports_from` [EXTRACTED]
- [[pydantic]] - `imports_from` [EXTRACTED]
- [[test_card_verification_records.py]] - `imports_from` [EXTRACTED]
- [[test_identity_verification.py]] - `imports_from` [EXTRACTED]
- [[test_search_offer_integrity.py]] - `imports_from` [EXTRACTED]
- [[test_source_warnings.py]] - `imports_from` [EXTRACTED]
- [[typing]] - `imports_from` [EXTRACTED]
- [[wb-connectorteststest_parser_live.py]] - `imports_from` [EXTRACTED]
- [[wb_connectorserver.py]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/wb_connector/serverpy