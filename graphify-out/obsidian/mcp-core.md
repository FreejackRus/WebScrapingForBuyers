---
source_file: "mcp-servers/ru-marketplace-mcp/packages/mcp-core/pyproject.toml"
type: "code"
community: "marketplace-connector"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/marketplace-connector
---

# mcp-core

## Connections
- [[aliexpress-connector]] - `depends_on` [EXTRACTED]
- [[avito-connector]] - `depends_on` [EXTRACTED]
- [[cian-connector]] - `depends_on` [EXTRACTED]
- [[citilink-connector]] - `depends_on` [EXTRACTED]
- [[compare-connector]] - `depends_on` [EXTRACTED]
- [[detmir-connector]] - `depends_on` [EXTRACTED]
- [[dns-connector]] - `depends_on` [EXTRACTED]
- [[lamoda-connector]] - `depends_on` [EXTRACTED]
- [[marketplace-connector]] - `depends_on` [EXTRACTED]
- [[megamarket-connector]] - `depends_on` [EXTRACTED]
- [[mpstats-connector]] - `depends_on` [EXTRACTED]
- [[ozon-connector]] - `depends_on` [EXTRACTED]
- [[ru-marketplace-mcp]] - `depends_on` [EXTRACTED]
- [[taobao-connector]] - `depends_on` [EXTRACTED]
- [[wb-connector]] - `depends_on` [EXTRACTED]
- [[yandex-connector]] - `depends_on` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/marketplace-connector