---
source_file: "mcp-servers/ru-marketplace-mcp/packages/mcp-core/tests/test_handoff_reporting.py"
type: "code"
community: "Community 87"
location: "L110"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_87
---

# test_the_resume_that_clears_the_challenge_reports_both_facts()

## Connections
- [[The interesting case same owned page, challenge gone, data moved on.]] - `rationale_for` [EXTRACTED]
- [[blocked_then_clear()]] - `indirect_call` [INFERRED]
- [[call()_2]] - `calls` [EXTRACTED]
- [[test_handoff_reporting.py]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_87