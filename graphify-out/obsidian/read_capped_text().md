---
source_file: "mcp-servers/ru-marketplace-mcp/packages/mcp-core/src/mcp_core/transport/http_tier.py"
type: "code"
community: "Community 3"
location: "L97"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_3
---

# read_capped_text()

## Connections
- [[BodyTooLargeError]] - `calls` [EXTRACTED]
- [[Response]] - `references` [EXTRACTED]
- [[Stream ``response`` into text, refusing to buffer past ``max_bytes``. The…]] - `rationale_for` [EXTRACTED]
- [[get_text_with_retries()]] - `calls` [EXTRACTED]
- [[http_tier.py]] - `contains` [EXTRACTED]
- [[transport__init__.py]] - `imports` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_3