---
source_file: "mcp-servers/ru-marketplace-mcp/packages/mpstats-connector/tests/test_call_envelope.py"
type: "code"
community: "Community 144"
location: "L61"
tags:
  - graphify/code
  - graphify/INFERRED
  - community/Community_144
---

# payload()

## Connections
- [[_attempt()_3]] - `indirect_call` [INFERRED]
- [[fake_call()]] - `indirect_call` [INFERRED]
- [[fake_call()_1]] - `indirect_call` [INFERRED]
- [[fake_call()_8]] - `indirect_call` [INFERRED]
- [[fake_call()_4]] - `indirect_call` [INFERRED]
- [[fake_card()]] - `indirect_call` [INFERRED]
- [[fake_fetch()_6]] - `indirect_call` [INFERRED]
- [[fake_fetch()_7]] - `indirect_call` [INFERRED]
- [[fake_fetch()_9]] - `indirect_call` [INFERRED]
- [[fake_post()_2]] - `indirect_call` [INFERRED]
- [[fake_render()_4]] - `indirect_call` [INFERRED]
- [[fake_render()]] - `indirect_call` [INFERRED]
- [[fake_render()_1]] - `indirect_call` [INFERRED]
- [[fake_render()_6]] - `indirect_call` [INFERRED]
- [[fake_render()_5]] - `indirect_call` [INFERRED]
- [[fake_render()_2]] - `indirect_call` [INFERRED]
- [[fake_render()_3]] - `indirect_call` [INFERRED]
- [[json()]] - `indirect_call` [INFERRED]
- [[test_a_transport_exception_does_not_carry_the_token()]] - `calls` [EXTRACTED]
- [[test_call_envelope.py]] - `contains` [EXTRACTED]
- [[test_html_body_reads_as_a_block_not_as_drift()]] - `calls` [EXTRACTED]
- [[test_http_429_is_rate_limited()]] - `calls` [EXTRACTED]
- [[test_inner_403_behind_http_200_is_auth_missing()]] - `calls` [EXTRACTED]
- [[test_inner_500_behind_http_200_is_transport_down()]] - `calls` [EXTRACTED]
- [[test_json_array_instead_of_object_is_parser_drift()]] - `calls` [EXTRACTED]
- [[test_no_token_fails_before_any_request()]] - `calls` [EXTRACTED]
- [[test_non_json_body_is_parser_drift()]] - `calls` [EXTRACTED]

#graphify/code #graphify/INFERRED #community/Community_144