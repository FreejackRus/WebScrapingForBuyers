---
source_file: "mcp-servers/ru-marketplace-mcp/packages/mcp-core/src/mcp_core/resilience.py"
type: "code"
community: "Community 66"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_66
---

# resilience.py

## Connections
- [[Tolerant-reader resilience helpers shared across marketplace connectors.…]] - `rationale_for` [EXTRACTED]
- [[_parse_money_string()]] - `contains` [EXTRACTED]
- [[_type_tag()]] - `contains` [EXTRACTED]
- [[_walk_shape()]] - `contains` [EXTRACTED]
- [[aliexpress-connectorteststest_shape_reference.py]] - `imports_from` [EXTRACTED]
- [[attach_meta()]] - `contains` [EXTRACTED]
- [[avito-connectorteststest_shape_reference.py]] - `imports_from` [EXTRACTED]
- [[cian-connectorteststest_shape_reference.py]] - `imports_from` [EXTRACTED]
- [[citilink-connectorteststest_shape_reference.py]] - `imports_from` [EXTRACTED]
- [[coerce_int()]] - `contains` [EXTRACTED]
- [[coerce_price()]] - `contains` [EXTRACTED]
- [[coerce_rating()]] - `contains` [EXTRACTED]
- [[collections_abc]] - `imports_from` [EXTRACTED]
- [[deep_first()]] - `contains` [EXTRACTED]
- [[detmir-connectorteststest_shape_reference.py]] - `imports_from` [EXTRACTED]
- [[diagnose_drift.py]] - `imports_from` [EXTRACTED]
- [[diff_keys()]] - `contains` [EXTRACTED]
- [[dns-connectorteststest_shape_reference.py]] - `imports_from` [EXTRACTED]
- [[dom.py]] - `imports_from` [EXTRACTED]
- [[first_present()]] - `contains` [EXTRACTED]
- [[flatten_text()]] - `contains` [EXTRACTED]
- [[lamoda-connectorteststest_shape_reference.py]] - `imports_from` [EXTRACTED]
- [[math]] - `imports` [EXTRACTED]
- [[megamarket-connectorteststest_shape_reference.py]] - `imports_from` [EXTRACTED]
- [[mpstats-connectorteststest_shape_reference.py]] - `imports_from` [EXTRACTED]
- [[ozon-connectorteststest_shape_reference.py]] - `imports_from` [EXTRACTED]
- [[price_from_texts()]] - `contains` [EXTRACTED]
- [[re]] - `imports` [EXTRACTED]
- [[selfcheck_entry()]] - `contains` [EXTRACTED]
- [[selfcheck_result()]] - `contains` [EXTRACTED]
- [[shape_signature()]] - `contains` [EXTRACTED]
- [[taobao-connectorteststest_shape_reference.py]] - `imports_from` [EXTRACTED]
- [[test_resilience.py]] - `imports_from` [EXTRACTED]
- [[test_resilience_properties.py]] - `imports_from` [EXTRACTED]
- [[test_search_parser_live.py]] - `imports_from` [EXTRACTED]
- [[test_shape_signature.py]] - `imports_from` [EXTRACTED]
- [[typing]] - `imports_from` [EXTRACTED]
- [[validate_offer()]] - `contains` [EXTRACTED]
- [[validate_review_block()]] - `contains` [EXTRACTED]
- [[wb-connectorteststest_shape_reference.py]] - `imports_from` [EXTRACTED]
- [[widget_prefixes()]] - `contains` [EXTRACTED]
- [[yandex-connectorteststest_shape_reference.py]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_66