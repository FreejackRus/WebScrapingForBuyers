---
source_file: "apps/web/src/widgets/search/offers/ui/index.tsx"
type: "code"
community: "offers/ui/index.tsx"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/offers/ui/indextsx
---

# offers/ui/index.tsx

## Connections
- [[OfferSortColumn]] - `imports` [EXTRACTED]
- [[OfferTable()]] - `contains` [EXTRACTED]
- [[columns]] - `contains` [EXTRACTED]
- [[conditionLabels]] - `imports` [EXTRACTED]
- [[emptySelected]] - `contains` [EXTRACTED]
- [[entitiesanalysisindex.ts]] - `imports_from` [EXTRACTED]
- [[entitiessearchindex.ts]] - `imports_from` [EXTRACTED]
- [[featuressearchindex.ts]] - `imports_from` [EXTRACTED]
- [[fetchedTime]] - `imports` [EXTRACTED]
- [[matchLabels]] - `imports` [EXTRACTED]
- [[money]] - `imports` [EXTRACTED]
- [[offerindex.ts]] - `imports_from` [EXTRACTED]
- [[offersindex.ts]] - `re_exports` [EXTRACTED]
- [[sharedlibindex.ts]] - `imports_from` [EXTRACTED]
- [[sortMark()]] - `contains` [EXTRACTED]
- [[useAnalysisStore]] - `imports` [EXTRACTED]
- [[useFilteredOffers()]] - `imports` [EXTRACTED]
- [[useOfferTable()]] - `imports` [EXTRACTED]
- [[useSearchStore]] - `imports` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/offers/ui/indextsx