---
type: community
cohesion: 0.83
members: 4
---

# test_fetch_composer_reports_cdp_navigation_block_as_blocked

**Cohesion:** 0.83 - tightly connected
**Members:** 4 nodes

## Members
- [[blocked_cdp()]] - code - mcp-servers/ru-marketplace-mcp/packages/ozon-connector/tests/test_server.py
- [[blocked_get()]] - code - mcp-servers/ru-marketplace-mcp/packages/ozon-connector/tests/test_server.py
- [[scenario()_62]] - code - mcp-servers/ru-marketplace-mcp/packages/ozon-connector/tests/test_server.py
- [[test_fetch_composer_reports_cdp_navigation_block_as_blocked()]] - code - mcp-servers/ru-marketplace-mcp/packages/ozon-connector/tests/test_server.py

## Live Query (requires Dataview plugin)

```dataview
TABLE source_file, type FROM #community/test_fetch_composer_reports_cdp_navigation_block_as_blocked
SORT file.name ASC
```

## Connections to other communities
- 1 edge to [[_COMMUNITY__run]]
- 1 edge to [[_COMMUNITY_ozon-connectorteststest_server.py]]

## Top bridge nodes
- [[test_fetch_composer_reports_cdp_navigation_block_as_blocked()]] - degree 5, connects to 2 communities