---
source_file: "mcp-servers/ru-marketplace-mcp/packages/compare-connector/src/compare_connector/vision_policy.py"
type: "code"
community: "Community 73"
location: "L64"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_73
---

# client_vision_hint()

## Connections
- [[An explicit vision hint from the client, or ``None`` when it says nothing.…]] - `rationale_for` [EXTRACTED]
- [[compare_browser_snapshot()]] - `calls` [EXTRACTED]
- [[compare_connectorserver.py]] - `imports` [EXTRACTED]
- [[test_a_client_that_says_nothing_is_unknown_not_visionless()]] - `calls` [EXTRACTED]
- [[test_client_hint_is_read_from_the_capability_extra_bags()]] - `calls` [EXTRACTED]
- [[vision_policy.py]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_73