---
source_file: "apps/identity/src/http/session.ts"
type: "code"
community: "session.ts"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/sessionts
---

# session.ts

## Connections
- [[AuthStore]] - `imports` [EXTRACTED]
- [[auth-store.ts]] - `imports_from` [EXTRACTED]
- [[clearSession()]] - `contains` [EXTRACTED]
- [[identitysrchttproutes.ts]] - `imports_from` [EXTRACTED]
- [[readSessionUser()]] - `contains` [EXTRACTED]
- [[ref_fastify]] - `imports_from` [EXTRACTED]
- [[ref_fastify_cookie]] - `imports_from` [EXTRACTED]
- [[ref_node_crypto]] - `imports_from` [EXTRACTED]
- [[sign()]] - `contains` [EXTRACTED]
- [[signSession()]] - `contains` [EXTRACTED]
- [[verifySession()]] - `contains` [EXTRACTED]
- [[writeSession()]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/sessionts